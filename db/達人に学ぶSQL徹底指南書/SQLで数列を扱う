...
## ポイント
- SQLでのデータを扱い方は2種類ある
- 1つは、順序を無視した集合と見なす方法。この方法は伝統的なSQLの集合と述語による考え方に基づいている
- もう一つは、順序を持った集合と見なす方法。この時の基本的な指針は、ウィンドウ関数によるダイレクトな順序の操作
- SQLで全称文を記述したいときは、存在文の否定に同値変換して、NOT EXISTS 述語を使う必要がある。これは、SQLが述語論理の存在量子化しか実装していないため。
