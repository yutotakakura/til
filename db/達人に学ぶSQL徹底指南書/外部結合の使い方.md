...
- 必要に迫られたときは外部結合やCASE式を駆使して乗り切ろう。
- 入れ子の表側を作るときはマスタの直積を作ってから結合一発でキメる。
- 行数に着目した場合、結合は「かけ算」として考えられる。だから、一体多の結合の場合、結果の行数は増えない。
...
```
上記の結果になった理由は、TblPopには年齢階級「2」のデータは存在しないため、年齢マスタと比較するときNULLとなり、unknownとなるため、結果が返らない。<br>
解決策は、外部結合を二度することが許されないなら、一度で済ませてしまえばいい。
```
...
SELECT I.item_no, SUM(SH.quantity) AS total_qty
 ...
 GROUP BY I.item_no;
```
その2の方が、中間ビューでメモリを使わないのとインデックスを効かすことができるので、パフォーマンスがいい。<br>
結合は、一対一でなくても一対多ならば行数は不当には増えない。
...
FROM Class_A ca LEFT JOIN Class_B cb 
ON ca.id = cb.id 
...
FROM Class_A ca RIGHT JOIN Class_B cb 
ON ca.id = cb.id 
...
-- 完全外部結合で排他的集合を求める
SELECT 
	COALESCE (ca.id, cb.id) AS id,
	COALESCE (ca.name, cb.name) AS name
FROM 
	Class_A ca FULL OUTER JOIN Class_B cb 
	ON ca.id = cb.id 
WHERE ca.name IS NULL
	OR cb.name IS NULL;
```
